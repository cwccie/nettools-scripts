...
# ---------------------------------------------------
# 10. Create Startup and Service Scripts
# ---------------------------------------------------
echo -e "\n${YELLOW}10. Creating Startup Scripts${NC}"

# Create start_nettools.sh
...
# ---------------------------------------------------
echo -e "\n${YELLOW}11. Setting Final Permissions${NC}"
...
# ---------------------------------------------------
echo -e "\n${YELLOW}12. Starting nettools service${NC}"
...
# ---------------------------------------------------
echo -e "\n${YELLOW}13. Checking Firewall Configuration${NC}"
...
# ---------------------------------------------------
echo -e "${GREEN}=========================================================${NC}"
echo -e "${GREEN}     nettools Setup Complete!                            ${NC}"
echo -e "${GREEN}=========================================================${NC}"
echo -e "\n${YELLOW}The service has been installed and started.${NC}"
...
#
# This script performs a complete setup of the nettools application with all fixes applied.
# It installs required packages, creates necessary users and directories, and configures
...
LOG_FILE="${BASE_DIR}/setup_log.txt"
...
echo -e "${GREEN}=========================================================${NC}"
echo -e "${GREEN}     nettools Complete Setup with Fixes                  ${NC}"
...
apt-get install -y net-tools tshark
...
if id "nettools_user" &>/dev/null; then
    ...
    adduser --system --shell /bin/bash --group --disabled-password --home /opt/nettools nettools_user
...
mkdir -p ${BASE_DIR}/{app/{auth,capture,dashboard,api,utils}/templates,app/static/{css,js,img},tests/{unit,integration,fixtures},pcap_files,logs}

# ---------------------------------------------------
# 4. Permissions Configuration
# ---------------------------------------------------
echo -e "\n${YELLOW}4. Permissions Configuration${NC}"
...
# ---------------------------------------------------
echo -e "\n${YELLOW}5. Python Virtual Environment Setup${NC}"

echo -e "${GREEN}-> Creating and activating virtual environment...${NC}"
cd ${BASE_DIR}
python3 -m venv venv
chown -R nettools_user:nettools_user ${BASE_DIR}/venv

# ---------------------------------------------------
# 6. Install Python Packages
# ---------------------------------------------------
echo -e "\n${YELLOW}6. Installing Python Packages${NC}"

su - nettools_user << 'EOF'
cd /opt/nettools
source venv/bin/activate
...
# ---------------------------------------------------
echo -e "\n${YELLOW}7. Creating Configuration Files${NC}"
...
# Create run.py with socket.io fix
...
# Create app/__init__.py with fixed initialization
...
# ---------------------------------------------------
# 8. Create Core Module Files
# ---------------------------------------------------
echo -e "\n${YELLOW}8. Creating Core Module Files${NC}"
...
# ---------------------------------------------------
# 9. Create Capture Module
# ---------------------------------------------------
echo -e "\n${YELLOW}9. Creating Capture Module${NC}"

# Create capture/__init__.py
touch ${BASE_DIR}/app/capture/__init__.py

# Create capture/routes.py
